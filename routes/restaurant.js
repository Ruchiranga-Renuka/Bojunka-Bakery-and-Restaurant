...
router.post('/login', (req, res) => {
  ...
  // naive login stub
  const token = generateToken('restaurant', username || 'user');
  res.json({ token });
});

router.get('/foods', (req, res) => {
  res.json(store.restaurantFoods);
});

router.get('/foods/:id/availability', (req, res) => {
  const id = Number(req.params.id);
  const item = store.restaurantFoods.find(f => f.id === id);
...
router.post('/orders', (req, res) => {
  ...
  const order = {
    id: store.restaurantOrders.length + 1,
    item: { id: item.id, name: item.name, price: item.price },
    quantity,
    customer: customer || 'anonymous',
    createdAt: new Date().toISOString()
  };
  store.restaurantOrders.push(order);
  res.status(201).json(order);
});

router.get('/orders', (req, res) => {
  res.json(store.restaurantOrders);
});
